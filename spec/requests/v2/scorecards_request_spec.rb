...
RSpec.describe V2::ScorecardsController, type: :request do
  describe "GET #show" do
    ...
    before {
      get "/api/v2/scorecards/#{scorecard.uuid}", headers: headers
    }
  ...
  describe "PUT #update" do
    ...
    context "success" do
      before {
        put "/api/v2/scorecards/#{scorecard.uuid}", params: { scorecard: params }, headers: headers
      }
    ...
    context "scorecard is not found" do
      before {
        put "/api/v2/scorecards/#{scorecard.uuid}abc", params: { scorecard: params }, headers: headers
      }
  ...
  describe "GET #show, pdf" do
    ...
    context "scorecard is finished" do
      before {
        scorecard.completed_by(user)
        get "/api/v2/scorecards/#{scorecard.uuid}.pdf", headers: headers
      }
    ...
    context "pass params locale" do
      ...
      context "locale is :en" do
        before {
          get "/api/v2/scorecards/#{scorecard.uuid}.pdf?locale=en", headers: headers
        }
      ...
      context "locale is :km" do
        before {
          get "/api/v2/scorecards/#{scorecard.uuid}.pdf?locale=km", headers: headers
        }
  ...
  describe "PUT #update, raised_indicator and voting_indicators" do
    ...
    let!(:custom_indicator) {create(:indicator, type: 'CscCore::Indicators::CustomIndicator', categorizable: facility)}
    let!(:custom_indicator2) {create(:indicator, type: 'CscCore::Indicators::CustomIndicator', categorizable: facility)}
    ...
    let(:params)      { { raised_indicators_attributes: [
                            ...
                            { indicatorable_id: custom_indicator.id, indicatorable_type: 'Indicators::CustomIndicator', scorecard_uuid: scorecard.uuid, voting_indicator_uuid: "124", selected: true },
                            { indicatorable_id: custom_indicator2.id, indicatorable_type: 'CscCore::Indicators::CustomIndicator', scorecard_uuid: scorecard.uuid, voting_indicator_uuid: "125", selected: true },
                          ],
                          voting_indicators_attributes: [
                            ...
                            { uuid: "124", indicatorable_id: custom_indicator.id, indicatorable_type: 'Indicators::CustomIndicator', scorecard_uuid: scorecard.uuid, display_order: 2 },
                            { uuid: "125", indicator_uuid: custom_indicator2.uuid, indicatorable_id: custom_indicator2.id, indicatorable_type: 'CscCore::Indicators::CustomIndicator', scorecard_uuid: scorecard.uuid, display_order: 2 },
                          ]
                        }
                      }
  ...
  describe "PUT #update, participants with profiles" do
    let!(:profiles)   { CscCore::Profile.create_defaults }
    let!(:user)       { create(:user) }
    let!(:scorecard)  { create(:scorecard, program: user.program) }
    let!(:id_poor)    { CscCore::Profile.find_by code: 'PO' }
    let!(:youth)      { CscCore::Profile.find_by code: 'YO' }
    let(:headers)     { { "ACCEPT" => "application/json", "Authorization" => "Token #{user.authentication_token}" } }
    let(:params)      { { participants_attributes: [
                            { uuid: "123", age: 19, gender: 'female', scorecard_uuid: scorecard.uuid, profile_ids: [id_poor.id, youth.id] },
                          ]
                        }
                      }
    ...
    context "success" do
      ...
      it { expect(scorecard.participants.length).to eq(1) }
      it { expect(scorecard.participants.first.profiles.length).to eq(2) }
    end
  end
end
