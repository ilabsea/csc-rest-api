...
RSpec.describe V2::IndicatorsController, type: :request do
  describe "GET #index" do
    let!(:user) { create(:user) }
    let!(:facility) { create(:facility, :with_indicators) }
    let!(:custom_indicator) {
      facility.indicators.create(name: "Custom", type: "CscCore::Indicators::CustomIndicator")
    }
    ...
    before {
      ...
      get "/api/v2/facilities/#{facility.id}/indicators", headers: headers
    }
